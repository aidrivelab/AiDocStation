# -*- coding: utf-8 -*-
"""
@File    : local_bridge/service/document/generator.py
...
def _get_default_reference_docx() -> Optional[str]:
    ...
    ref_path = resource_path("pandoc/Reference-document.docx")
...
class DocumentGenerator:
    ...
    def _get_effective_reference_docx(self, config: dict) -> (Optional[str], str):
        ...
        if mode == "built-in":
            path = _get_default_reference_docx()
            return path, "built-in"
            
        if mode == "custom":
            path = config.get("reference_docx")
            if path and os.path.exists(path):
                return path, "custom"
            else:
                log(f"[Generator] Custom path invalid or missing: {path}, falling back to None")
                return None, "custom_invalid"
    ...
    def convert_markdown_to_docx_bytes(self, md_text: str, config: dict) -> bytes:
        ...
        from ...integrations.pandoc import LUA_DOCX_BODY_MAP, LUA_LIST_TO_TEXT
        if LUA_DOCX_BODY_MAP not in filters:
            filters.append(LUA_DOCX_BODY_MAP)
        

        if config.get("list_handle_method") == "keep":
            if LUA_LIST_TO_TEXT not in filters:
                filters.append(LUA_LIST_TO_TEXT)
                log(f"[Generator] Enabled List-to-Text Lua filter")
        ...
        mapping_method = config.get("style_mapping_method", "post_processing")
        log(f"[Generator] Style mapping method: {mapping_method}")
        

        reference_docx, ref_mode = self._get_effective_reference_docx(config)
        log(f"[Generator] Reference mode: {ref_mode}, final path: {reference_docx}")


        metadata = {}
        if mapping_method == "reference_doc":
    ...
    def convert_html_to_docx_bytes(self, html_text: str, config: dict) -> bytes:
        ...
        reference_docx, ref_mode = self._get_effective_reference_docx(config)
        log(f"[Generator] Reference mode (HTML): {ref_mode}, path: {reference_docx}")
        
        body_style = config.get("body_style", "正文")
        metadata = {}
        if ref_mode != "disabled":
            ...
            log("[Generator] HTML Isolation: Mode IS disabled, cleaning metadata to avoid style carry-over.")
    ...
    def convert_docx_to_html_text(self, docx_path: str) -> str:
        ...
        self._ensure_pandoc_integration()
        return self._pandoc_integration.convert_docx_to_html_text(docx_path)
